 ...
 */
static uint8_t
vFTPConnection(struct netconn *connfd, uint8_t *alloc_rq)
{
  //uint8 temp_data;
  ...
  netconn_write(connfd,FTP_WELCOME_RESPONSE,str_len(FTP_WELCOME_RESPONSE),0);
  
  do
  {
    /*if reception is OK: wait for REQUEST from client*/
    netconn_rcv_req((void *)connfd, alloc_rq, NULL,0);
    ...
    {
       /*PORT request*/
       if( strstr((char *)alloc_rq,FTP_PORT_REQUEST) != NULL )
       {
          FTP_DataFlowControl(connfd,&alloc_rq[sizeof(FTP_PORT_REQUEST)]);
       }
       /*DELETE request*/
       else if( strstr((char *)alloc_rq,FTP_DELE_REQUEST) != NULL )
       {
          //if (FILE_DELETED == FAT_FileDelete(SD_FTP_Handle,SDbuffer,(uint8 *)strupr(&alloc_rq[sizeof(FTP_DELE_REQUEST)])) )
    	   if (cfs_remove(&alloc_rq[sizeof(FTP_DELE_REQUEST)]) == 0)
          {
             //****RESPONSE: OK
             netconn_write(connfd,FTP_DELE_OK_RESPONSE,str_len(FTP_DELE_OK_RESPONSE),0);
          }
          else

          {
             //****RESPONSE: FAILED
             netconn_write(connfd,FTP_WRITE_FAIL_RESPONSE,str_len(FTP_WRITE_FAIL_RESPONSE),0);
          }
       }
       /*SYST request*/
       else if( strstr((char *)alloc_rq,FTP_SYST_REQUEST) != NULL )
		{
			netconn_write(connfd,FTP_SYST_OK_RESPONSE,str_len(FTP_SYST_OK_RESPONSE),0);
		}
       /*FEAT request*/
       else if( strstr((char *)alloc_rq,FTP_FEAT_REQUEST) != NULL )
		{
			netconn_write(connfd,FTP_FEAT_OK_RESPONSE,str_len(FTP_FEAT_OK_RESPONSE),0);
			netconn_write(connfd,FTP_FEAT_END_RESPONSE,str_len(FTP_FEAT_END_RESPONSE),0);
		}
       /*HELP request*/
       else if( strstr((char *)alloc_rq,FTP_HELP_REQUEST) != NULL )
		{
			netconn_write(connfd,FTP_UNKNOWN_RESPONSE,str_len(FTP_UNKNOWN_RESPONSE),0);
		}
       /*OPTS request*/
       else if( strstr((char *)alloc_rq,FTP_OPTS_REQUEST) != NULL )
		{
    	   netconn_write(connfd,FTP_OPTS_OK_RESPONSE,str_len(FTP_OPTS_OK_RESPONSE),0);
		}
       /*PWD request*/
       else if( strstr((char *)alloc_rq,FTP_PWD_REQUEST) != NULL )
		{
    	   netconn_write(connfd,FTP_PWD_RESPONSE,str_len(FTP_PWD_RESPONSE),0);
		}
       /*TYPE A request*/
       else if( strstr((char *)alloc_rq,FTP_TYPE_REQUEST) != NULL )
		{
    	   netconn_write(connfd,FTP_OPTS_OK_RESPONSE,str_len(FTP_OPTS_OK_RESPONSE),0);
		}
    ...
    {
       if( strstr((char *)alloc_rq,FTP_USER_REQUEST) != NULL )
       {          
          ...
          {
              //*****RESPONSE USER OK
              netconn_write(connfd,FTP_USER_RESPONSE,str_len(FTP_USER_RESPONSE),0);
              ...
              if( strstr((char *)alloc_rq,FTP_PASS_REQUEST) != NULL )
              {
                  ...
                  if( !strncmp(&alloc_rq[sizeof(FTP_PASS_REQUEST)],FTP_PASSWORD,str_len(FTP_PASSWORD)) )
                  {
                    ...
                    netconn_write(connfd,FTP_PASS_OK_RESPONSE,str_len(FTP_PASS_OK_RESPONSE),0);
                  ...
                  else
                  {
                    ...
                    netconn_write(connfd,FTP_PASS_FAIL_RESPONSE,str_len(FTP_PASS_FAIL_RESPONSE),0);
                  }
              }
              else
              {
                  //***RESPONSE: EXPECTING PASS request
                  netconn_write(connfd,FTP_BAD_SEQUENCE_RESPONSE,str_len(FTP_BAD_SEQUENCE_RESPONSE),0);
              }
          }
          else
          {
...
{
   if( strstr((char *)alloc_rq,FTP_QUIT_REQUEST) != NULL )
   {
 ...
 */
static uint8_t
FTP_DataFlowControl(struct netconn *connfd, uint8_t *alloc_rq)
{
    ...
    for(i=0;i<sizeof(ip_address);i++)
    {
       ...
       ip_address.bytes[i] = (uint8_t)strtoul((char *)alloc_rq,(char **)&end,10);
       alloc_rq = (uint8_t *)end + 1;
    }
    //Get FTP Port:first part
    end = (CHAR *)strchr((const CHAR *)alloc_rq,',');        
    ftp_port.u8[1] = (uint8_t)strtoul((char *)alloc_rq,(char **)&end,10);
    alloc_rq = (uint8_t *)end + 1;
    //Get FTP Port:second part
    end = (CHAR *)strchr((const CHAR *)alloc_rq,'\r');        
    ftp_port.u8[0] = (uint8_t)strtoul((char *)alloc_rq,(char **)&end,10);
    /*****END**********************************/
     
    //*****RESPONSE: OPEN PORT
    netconn_write(connfd,FTP_PORT_OK_RESPONSE,str_len(FTP_PORT_OK_RESPONSE),0);
    ...
    /*NLST*/
    if(( strstr((char *)alloc_rq,FTP_NLST_REQUEST) != NULL ) || ( strstr((char *)alloc_rq,FTP_LIST_REQUEST) != NULL ))
    {
      if( (conn_data = FTP_OpenDataPort((struct ip_addr *)&(ip_address.lword),ftp_port.u16)) != NULL )
      {
        //****Response Sending
        netconn_write(connfd,FTP_NLST_OK_RESPONSE,str_len(FTP_NLST_OK_RESPONSE),0);
        ...
        //****RESPONSE: OK
        netconn_write(connfd,FTP_TRANSFER_OK_RESPONSE,str_len(FTP_TRANSFER_OK_RESPONSE),0);
      }
      else
      {
        //****RESPONSE PORT cant be opened!!
        netconn_write(connfd,FTP_DATA_PORT_FAILED,str_len(FTP_DATA_PORT_FAILED),0);
        
        return 1;/*FAIL*/        
      }
    }
    /*READ*/
    else if( strstr((char *)alloc_rq,FTP_RETR_REQUEST) != NULL )
    {
      if( !FTP_Read_Is_Possible(&alloc_rq[sizeof(FTP_RETR_REQUEST)]) )
      {
        ...
        {
          //****Response Sending
          netconn_write(connfd,FTP_RETR_OK_RESPONSE,str_len(FTP_RETR_OK_RESPONSE),0);
    ...
    /*WRITE*/
    else if( strstr((char *)alloc_rq,FTP_STOR_REQUEST) != NULL )
    {
      ...
      else
      {
        ...
        {
          //****Response Sending
          netconn_write(connfd,FTP_STOR_OK_RESPONSE,str_len(FTP_STOR_OK_RESPONSE),0);
    ...
    else
    {
      ...
      netconn_write(connfd,FTP_CMD_NOT_IMP_RESPONSE,str_len(FTP_CMD_NOT_IMP_RESPONSE),0);
    }
...
FTP_SD_send(void* var,UINT8 *fileName)
{
    ...
    /*append "\r\n"*/
    strcat((char *) fileName, STRING_END );
    /*send files name*/
    netconn_write(connfd,fileName,strlen((char *)fileName),NETCONN_COPY);
    return 1;  
}
...
FTP_CompareFile(void* var,UINT8 *fileName)
{
    ...
    if( !strcmp((char *)string,(const char *)SDbuffer) )
      return 0;/*File Found*/
    else
    ...
    //(void)FAT_LS(SD_FTP_Handle,SDbuffer,connfd,FTP_SD_send);
     ...
     if(cfs_opendir(&dir, "/") == 0) {
       while(cfs_readdir(&dir, &dirent) != -1) {
		 ...
		 /*append "\r\n"*/
		 strcat((char *) dirent.name, STRING_END );
		 /*send files name*/
		 netconn_write(connfd,dirent.name,dirent.size + sizeof(STRING_END),NETCONN_COPY);
...
FTP_Read_Is_Possible(uint8 *data)
{
  ...
  if (!cfs_open((char *)data, CFS_READ))
  {
     result = 0;/*OK*/
  }
...
FTP_Read_File(struct netconn *connfd)
{
  ...
  while( (u16Length=cfs_read(SD_FTP_Handle->FileDesc, SDbuffer, BLOCK_SIZE)) == BLOCK_SIZE )
  {     
...
FTP_Write_File(struct netconn *connfd, uint8 *data)
{
	...
	SD_FTP_Handle->FileDesc = cfs_open((char *)data, CFS_WRITE);
    if(SD_FTP_Handle->FileDesc > -1)
    {
       do
       {       
         ...
         if( connfd->last_err == ERR_OK )
         {
           ...
           {             
             ...
             cfs_write(SD_FTP_Handle->FileDesc, q->payload,q->len);
           }
           while( ( q = q->next ) != NULL );
       ...
       cfs_close(SD_FTP_Handle->FileDesc);
    }
...
*/
    ...
    for(;;)/*infinite loop*/
    {
      ...
      {
         if( netconn_accept(conn, &connection) == 0 )
         {
            i = FTP_OPEN;
             /*set timeout for this connection*/
...
{
	sys_thread_new("FTPServer", vBasicFTPServer, NULL, 1024, DEFAULT_THREAD_PRIO);
}
