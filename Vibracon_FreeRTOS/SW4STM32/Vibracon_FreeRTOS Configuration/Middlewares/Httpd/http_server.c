...
#endif          
     /*send header*/
     netconn_write(pcb, &array_to_send[0], (uint16)strlen((char *)array_to_send), NETCONN_COPY );
}
...
HTTP_SendData(struct netconn *pcb, uint8 dynamic_flag, uint8 *array_to_send, uint8 *dynamic_info, uint16 dynamic_info_length)
{
    ...
    else
    {
       ...
       netconn_write(pcb, &dynamic_info[0], (uint16)dynamic_info_length, NETCONN_NOCOPY);
#if HTTP_DYNAMIC_SUPPORT == 1
    }
#endif  
}
...
HTTP_ShowTasksStatus(struct netconn *pcb)
{
      ...
      netconn_write(pcb, HTTP_DYNAMIC_HEADER, ( u16_t )str_len( HTTP_DYNAMIC_HEADER ), NETCONN_NOCOPY );
...
{
     //header
     netconn_write(pcb, HTTP_WRONG_HEADER, str_len(HTTP_WRONG_HEADER), NETCONN_NOCOPY );
     //data
     netconn_write(pcb, HTTP_WRONG_DATA, str_len(HTTP_WRONG_DATA), NETCONN_NOCOPY );
}
...
HTTP_Server_Task( void *pvParameters )
{
    ...
    else
    {
      ...
      if( (u8Archivo= (uint8 *)mem_malloc( BLOCK_SIZE+1 )) != NULL )
      {
    ...
    {      
      for(i=0;i<HTTP_SESSIONS;i++)
      {
        ...
        {
        	netconn_accept(conn, &connections[i].connection);
...
{
	sys_thread_new("HTTPServer", HTTP_Server_Task, NULL, 128, DEFAULT_THREAD_PRIO);
}
